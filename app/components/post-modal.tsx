...
export function PostModal({ children, useBackNavigation = false }: PostModalProps) {
  ...
  return (
    ...
    >
      <article className="hide-scroll bg-neutral-50 dark:bg-[#111] border border-neutral-500/10 rounded-lg w-full max-w-sm max-h-[90vh] overflow-y-auto px-8 pb-12 mx-4 relative shadow-sm">
        <div className="sticky top-0 z-99 flex justify-end h-fit">
          <button
            onClick={handleClose}
            aria-label="Close"
            className="cursor-pointer mt-4 -mr-4 shadow-sm border-2 border-mist-500/30 rounded-full p-1 text-mist-500/80 bg-neutral-50 dark:bg-[#111]"
          >
            <svg
